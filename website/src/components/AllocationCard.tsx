...
export function AllocationCard({ allocation, project, wallet, isCreator, onUpdate }: Props) {
  ...
  const handleClaim = async () => {
    if (!wallet) return
    try {
      const result = await api.claimAllocation(project.id, allocation.id, wallet)
  ...
  const handleImpressions = async () => {
    const val = prompt('Enter current impression count:', String(
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {allocation.status === 'unlocked' && wallet === allocation.recipientWallet && (
          <button
            onClick={handleClaim}
            className="rounded-lg bg-[#4ade80] px-4 py-2 text-sm font-semibold text-[#0a0f0d] hover:bg-[#22c55e] transition-colors"
          >
            Claim Allocation
          </button>
        )}
        ...
        {isCreator && allocation.status !== 'claimed' && (
          <button
